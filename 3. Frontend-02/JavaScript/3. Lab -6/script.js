...
const arr = [1, 2, 3, 4];
// REDUCE METODU ILƏ
const funcReduce = (arr) =>
  arr.reduce((acc, item) => {
    acc.push(item * 2);

    return acc;
  }, []);
console.log(funcReduce([1, 2, 3, 4]));
// //
// MAP METODU İLƏ
...
console.log(funcForEach([1, 2, 3, 4]));
//  //
// forOf METODU ILƏ
const funcForOf = (arr) => {
  let newArr = [];
...
console.log(funcForOf([1, 2, 3, 4]));
// //
// FLATMAP METODU ILƏ
const funcFlatMap = (arr) => arr.flatMap((item) => item * 2);
console.log(funcFlatMap([1, 2, 3, 4]));
// //
// ARRAY FROM METODU ILƏ
const funcArrayFrom = (arr) => Array.from(arr, (item) => item * 2);

console.log(funcArrayFrom([1, 2, 3, 4]));

const result = {
  reduce: funcReduce(arr),
  map: funcMap(arr),
  forEach: funcForEach(arr),
  forOf: funcForOf(arr),
  flatMap: funcFlatMap(arr),
  arrayFrom: funcArrayFrom(arr),
};
